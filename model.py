...
class HOIST_with_static(nn.Module):
    ...
    def forward(self, dynamic, static, h0=None):
        ...
        if h0 is None:
            h0 = torch.randn(1, N, self.rnn_dim).to(self.device)
        h_dynamic, hn = self.rnn(total_weights*dynamic)
        
        static_embedding = self.static_fc(static)
        h = h_dynamic + static_embedding.unsqueeze(1).expand(-1, h_dynamic.size(1), -1)
        
        h_att = h.reshape(N,1,T,self.rnn_dim).repeat(1,N,1,1)
        h_att = h + (h_att * dist.reshape(N,N,1,1)).sum(1)
        y = self.linear(h_att)
        y = self.linear_2(F.leaky_relu(y))
        return y, [None, None, hn]